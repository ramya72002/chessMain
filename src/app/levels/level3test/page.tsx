...
import './Level3test.scss'; // Importing SCSS file for styling
...
const Level3Test = () => {
  ...
  const handleProceedToLevel3 = async () => {
    ...
    const response = await fetch('https://backend-chess-tau.vercel.app/updatelevel', {
        ...
        body: JSON.stringify({ name: userDetails.name, level: 'level3' }),
      ...
      if (result.success) {
        const userDetailsString = localStorage.getItem('userDetails');
        const userDetails = userDetailsString ? JSON.parse(userDetailsString) : null;        userDetails.level = 'level3';
        localStorage.setItem('userDetails', JSON.stringify(userDetails));
        console.log("l1",localStorage)
        // Navigate to portalhome
        router.push('/portalhome'); // Navigate to Level 3
  ...
  return (
    <div className="level2-test-container">
      <h1>Level 3 Test</h1>
      <p>Answer the following questions to proceed to Level 3:</p>
