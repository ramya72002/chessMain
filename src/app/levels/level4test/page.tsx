...
import './Level4test.scss'; // Importing SCSS file for styling
...
const Level4Test = () => {
  ...
  const handleProceedToLevel4 = async () => {
    ...
    const response = await fetch('https://backend-chess-tau.vercel.app/updatelevel', {
        ...
        body: JSON.stringify({ name: userDetails.name, level: 'level4' }),
    ...
    const userDetails = userDetailsString ? JSON.parse(userDetailsString) : null;
        ...
        router.push('/portalhome'); // Navigate to Level 4
  ...
  return (
    <div className="level2-test-container">
      <h1>Level 4 Test</h1>
      <p>Answer the following questions to proceed to Level 4:</p>
